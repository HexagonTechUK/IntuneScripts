<#
.SYNOPSIS
    Reports on disabled Azure AD users who still have active Microsoft 365 licenses assigned.

.DESCRIPTION
    This script connects to Azure Active Directory using the AzureAD module,
    identifies all disabled user accounts, checks for any assigned licenses,
...
.OUTPUTS
    CSV file saved to: C:\DisabledUsersWithLicenses.csv
    Log file saved to: C:\ProgramData\_MEM\DisabledLicensedAccount-Status.log

.NOTES
    File Name : Get-DisabledAccountsWithLicense.ps1  
...
.EXAMPLE
    Run the script as-is:
        .\Get-DisabledUsersWithLicenses.ps1
#>

# Variables (complete these):
$deployType  = "DisabledAccount"
$productName = "Status"
...
$disabledUsers = Get-AzureADUser -All $true | Where-Object { -not $_.AccountEnabled -and $_.AssignedLicenses.Count -gt 0 }
...
# Output
$results | Export-Csv -Path "C:\Admins\DisabledUsersWithLicenses.csv" -NoTypeInformation
